...
WHERE b.Expiry_Date < CURDATE()
  AND NOT EXISTS (
    ...
    WHERE aeb.Pharmacy_Name = i.Pharmacy_Name AND aeb.A_Expired_Batches = b.Batch_Number
  );
...
WHERE b.Expiry_Date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)
  AND NOT EXISTS (
    ...
    WHERE aueb.Pharmacy_Name = i.Pharmacy_Name AND aueb.A_Upcoming_expiring_Batches = b.Batch_Number
  );
...
SELECT i.Pharmacy_Name, b.Batch_Number
FROM BATCH b JOIN INVENTORY i ON b.Batch_Number = i.Batch_Number
WHERE b.Date_Added BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY)
    AND NOT EXISTS (
        SELECT 1 FROM ANALYTICS_NEWLY_ADDED_BATCHES anab
        WHERE anab.Pharmacy_Name = i.Pharmacy_Name AND anab.A_Newly_Added_Batches = b.Batch_Number
    );